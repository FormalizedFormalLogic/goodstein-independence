/-
# `Provable` — the control-ordinal operator witness-bounded `Z_∞` calculus (Towsner §15 + §19, lap-8)
...
This file: the inductive `Provable` + structural layer (`mono_k`, `mono_d`, `mono_c`, `mono_e`, `weakening`).
...
noncomputable def atomTrue (φ : ArithmeticFormula ℕ) : Prop := GoodsteinPA.Compat.gEvalm ℕ (fun _ => 0) (fun _ => 0) φ

/-- **The control-ordinal operator witness-bounded `Z_∞` calculus** `Provable α e k d c Γ`.
...
theorem mono_k : ∀ {α e k d c Γ}, Provable α e k d c Γ → ∀ {k'}, k ≤ k' → Provable α e k' d c Γ := by
  intro α e k d c Γ dd
  induction dd with
  | axL r v hp hn => intro k' _; exact Provable.axL r v hp hn
  | verumR h => intro k' _; exact Provable.verumR h
  | trueRel r v htrue hτ hαNF hmem =>
      intro k' hk; exact Provable.trueRel r v htrue (lt_of_lt_of_le hτ (by omega)) hαNF hmem
  | trueNrel r v htrue hτ hαNF hmem =>
      intro k' hk; exact Provable.trueNrel r v htrue (lt_of_lt_of_le hτ (by omega)) hαNF hmem
  | wk hsub _ ih => intro k' hk; exact Provable.wk hsub (ih hk)
  | weak hβ hβNF hαNF hτ hsub _ ih =>
      intro k' hk; exact Provable.weak hβ hβNF hαNF (lt_of_lt_of_le hτ (by omega)) hsub (ih hk)
  ...
  | orI φ ψ hβ hβNF hαNF hτ _ ih =>
      intro k' hk; exact Provable.orI φ ψ hβ hβNF hαNF (lt_of_lt_of_le hτ (by omega)) (ih hk)
...
theorem mono_d : ∀ {α e k d c Γ}, Provable α e k d c Γ → ∀ {d'}, d ≤ d' → Provable α e k d' c Γ := by
  intro α e k d c Γ dd
  induction dd with
  | axL r v hp hn => intro d' _; exact Provable.axL r v hp hn
  | verumR h => intro d' _; exact Provable.verumR h
  | trueRel r v htrue hτ hαNF hmem =>
      intro d' hd; exact Provable.trueRel r v htrue (lt_of_lt_of_le hτ (by omega)) hαNF hmem
  | trueNrel r v htrue hτ hαNF hmem =>
      intro d' hd; exact Provable.trueNrel r v htrue (lt_of_lt_of_le hτ (by omega)) hαNF hmem
  | wk hsub _ ih => intro d' hd; exact Provable.wk hsub (ih hd)
  | weak hβ hβNF hαNF hτ hsub _ ih =>
      intro d' hd; exact Provable.weak hβ hβNF hαNF (lt_of_lt_of_le hτ (by omega)) hsub (ih hd)
  | andI φ ψ hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ihφ ihψ =>
      intro d' hd
      exact Provable.andI φ ψ hβφ hβψ hβφNF hβψNF hαNF (lt_of_lt_of_le hτφ (by omega))
        (lt_of_lt_of_le hτψ (by omega)) (ihφ hd) (ihψ hd)
  | orI φ ψ hβ hβNF hαNF hτ _ ih =>
      intro d' hd; exact Provable.orI φ ψ hβ hβNF hαNF (lt_of_lt_of_le hτ (by omega)) (ih hd)
  | allω φ β hβ hβNF hαNF hτ _ ih =>
      intro d' hd
...
theorem mono_c : ∀ {α e k d c Γ}, Provable α e k d c Γ → ∀ {c'}, c ≤ c' → Provable α e k d c' Γ := by
  intro α e k d c Γ dd
  induction dd with
  | axL r v hp hn => intro c' _; exact Provable.axL r v hp hn
  | verumR h => intro c' _; exact Provable.verumR h
  | trueRel r v htrue hτ hαNF hmem => intro c' _; exact Provable.trueRel r v htrue hτ hαNF hmem
  | trueNrel r v htrue hτ hαNF hmem => intro c' _; exact Provable.trueNrel r v htrue hτ hαNF hmem
  | wk hsub _ ih => intro c' hc; exact Provable.wk hsub (ih hc)
  | weak hβ hβNF hαNF hτ hsub _ ih => intro c' hc; exact Provable.weak hβ hβNF hαNF hτ hsub (ih hc)
  | andI φ ψ hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ihφ ihψ =>
      intro c' hc; exact Provable.andI φ ψ hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ (ihφ hc) (ihψ hc)
  | orI φ ψ hβ hβNF hαNF hτ _ ih => intro c' hc; exact Provable.orI φ ψ hβ hβNF hαNF hτ (ih hc)
  | allω φ β hβ hβNF hαNF hτ _ ih => intro c' hc; exact Provable.allω φ β hβ hβNF hαNF hτ (fun n => ih n hc)
  | exI φ n hβ hβNF hαNF hτ hbound _ ih =>
      intro c' hc; exact Provable.exI φ n hβ hβNF hαNF hτ hbound (ih hc)
  | cut φ hcompl hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ih₁ ih₂ =>
      intro c' hc
...
theorem mono_e : ∀ {α e k d c Γ}, Provable α e k d c Γ → ∀ {e'}, e.NF → e'.NF → e < e' →
    norm e ≤ k + d → Provable α e' k d c Γ := by
  intro α e k d c Γ dd
  induction dd with
  | axL r v hp hn => intro e' _ _ _ _; exact Provable.axL r v hp hn
  | verumR h => intro e' _ _ _ _; exact Provable.verumR h
  | trueRel r v htrue hτ hαNF hmem => intro e' _ _ _ _; exact Provable.trueRel r v htrue hτ hαNF hmem
  | trueNrel r v htrue hτ hαNF hmem => intro e' _ _ _ _; exact Provable.trueNrel r v htrue hτ hαNF hmem
  | wk hsub _ ih => intro e' he heN' hlt hnorm; exact Provable.wk hsub (ih he heN' hlt hnorm)
  | weak hβ hβNF hαNF hτ hsub _ ih =>
      intro e' he heN' hlt hnorm; exact Provable.weak hβ hβNF hαNF hτ hsub (ih he heN' hlt hnorm)
  | andI φ ψ hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ihφ ihψ =>
  ...
  | orI φ ψ hβ hβNF hαNF hτ _ ih =>
      intro e' he heN' hlt hnorm; exact Provable.orI φ ψ hβ hβNF hαNF hτ (ih he heN' hlt hnorm)
  | allω φ β hβ hβNF hαNF hτ _ ih =>
      intro e' he heN' hlt hnorm
...
theorem orInv {φ ψ : ArithmeticFormula ℕ} : ∀ {α e k d c Γ}, Provable α e k d c Γ → (φ ⋎ ψ) ∈ Γ →
  ...
  | @andI α βφ' βψ' e k d c Γ₀ φ' ψ' hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ihφ ihψ =>
      ...
      have Pφ := Provable.wk (invPush (φ ⋎ ψ) φ' Γ₀) (ihφ (Finset.mem_insert_of_mem hmem0))
      have Pψ := Provable.wk (invPush (φ ⋎ ψ) ψ' Γ₀) (ihψ (Finset.mem_insert_of_mem hmem0))
  ...
  | @allω α e k d c Γ₀ χ β hβ hβNF hαNF hτ _ ih =>
      ...
      have key : ∀ n, Provable (β n) e (max k n) d c
        ...
        Provable.wk (invPush (φ ⋎ ψ) (χ/[nm n]) Γ₀) (ih n (Finset.mem_insert_of_mem hmem0))
  ...
  | @exI α β e k d c Γ₀ χ n hβ hβNF hαNF hτ hbound _ ih =>
      ...
      have hhead : (∃⁰ χ) ≠ (φ ⋎ ψ) := by intro h; simp [Vee.vee] at h
      have hmem0 : (φ ⋎ ψ) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
      have P := Provable.wk (invPush (φ ⋎ ψ) (χ/[nm n]) Γ₀) (ih (Finset.mem_insert_of_mem hmem0))
  ...
  | @cut α βφ' βψ' e k d c Γ₀ χ hcompl hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ih₁ ih₂ =>
      intro hmem
      have P₁ := Provable.wk (invPush (φ ⋎ ψ) χ Γ₀) (ih₁ (Finset.mem_insert_of_mem hmem))
      have P₂ := Provable.wk (invPush (φ ⋎ ψ) (∼χ) Γ₀) (ih₂ (Finset.mem_insert_of_mem hmem))
...
theorem andInvL {φ ψ : ArithmeticFormula ℕ} : ∀ {α e k d c Γ}, Provable α e k d c Γ → (φ ⋏ ψ) ∈ Γ →
  ...
  | @andI α βφ' βψ' e k d c Γ₀ φ' ψ' hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ dφ _ ihφ ihψ =>
      ...
      · have hmem0 : (φ ⋏ ψ) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhd e.symm
        have Pφ := Provable.wk (inv1Push (φ ⋏ ψ) _ φ' Γ₀) (ihφ (Finset.mem_insert_of_mem hmem0))
        have Pψ := Provable.wk (inv1Push (φ ⋏ ψ) _ ψ' Γ₀) (ihψ (Finset.mem_insert_of_mem hmem0))
  ...
  | @allω α e k d c Γ₀ χ β hβ hβNF hαNF hτ _ ih =>
      ...
      have key : ∀ n, Provable (β n) e (max k n) d c (insert (χ/[nm n]) (insert φ (Γ₀.erase (φ ⋏ ψ)))) :=
        fun n => Provable.wk (inv1Push (φ ⋏ ψ) _ (χ/[nm n]) Γ₀) (ih n (Finset.mem_insert_of_mem hmem0))
  ...
  | @exI α β e k d c Γ₀ χ n hβ hβNF hαNF hτ hbound _ ih =>
      ...
      have hmem0 : (φ ⋏ ψ) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
      have P := Provable.wk (inv1Push (φ ⋏ ψ) _ (χ/[nm n]) Γ₀) (ih (Finset.mem_insert_of_mem hmem0))
      exact Provable.wk (inv1Pull (φ ⋏ ψ) _ hhead Γ₀) (Provable.exI χ n hβ hβNF hαNF hτ hbound P)
  | @cut α βφ' βψ' e k d c Γ₀ χ hcompl hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ih₁ ih₂ =>
      intro hmem
      have P₁ := Provable.wk (inv1Push (φ ⋏ ψ) _ χ Γ₀) (ih₁ (Finset.mem_insert_of_mem hmem))
      have P₂ := Provable.wk (inv1Push (φ ⋏ ψ) _ (∼χ) Γ₀) (ih₂ (Finset.mem_insert_of_mem hmem))
      exact Provable.cut χ hcompl hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ P₁ P₂

/-- **∧-inversion, right** (Towsner §19.3): replace `φ ⋏ ψ` by `ψ`, same `(α,k,d,c)`. -/
theorem andInvR {φ ψ : ArithmeticFormula ℕ} : ∀ {α e k d c Γ}, Provable α e k d c Γ → (φ ⋏ ψ) ∈ Γ →
...
theorem allInv {φ₀ : ArithmeticSemiformula ℕ 1} (n₀ : ℕ) :
    ∀ {α e k d c Γ}, Provable α e k d c Γ → (∀⁰ φ₀) ∈ Γ →
      Provable α e (max k n₀) d c (insert (φ₀/[nm n₀]) (Γ.erase (∀⁰ φ₀))) := by
  ...
  | @andI α βφ' βψ' e k d c Γ₀ φ' ψ' hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ihφ ihψ =>
      ...
      have Pφ := Provable.wk (inv1Push (∀⁰ φ₀) _ φ' Γ₀) (ihφ (Finset.mem_insert_of_mem hmem0))
      have Pψ := Provable.wk (inv1Push (∀⁰ φ₀) _ ψ' Γ₀) (ihψ (Finset.mem_insert_of_mem hmem0))
  ...
  | @allω α e k d c Γ₀ χ β hβ hβNF hαNF hτ dd ih =>
      ...
      · have hmem0 : (∀⁰ φ₀) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhd e.symm
        have key : ∀ n, Provable (β n) e (max (max k n₀) n) d c
          ...
          intro n
          have h := Provable.wk (inv1Push (∀⁰ φ₀) _ (χ/[nm n]) Γ₀) (ih n (Finset.mem_insert_of_mem hmem0))
  ...
  | @exI α β e k d c Γ₀ χ n hβ hβNF hαNF hτ hbound _ ih =>
      ...
      have hmem0 : (∀⁰ φ₀) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
      have P := Provable.wk (inv1Push (∀⁰ φ₀) _ (χ/[nm n]) Γ₀) (ih (Finset.mem_insert_of_mem hmem0))
      exact Provable.wk (inv1Pull (∀⁰ φ₀) _ hhead Γ₀)
  ...
  | @cut α βφ' βψ' e k d c Γ₀ χ hcompl hβφ hβψ hβφNF hβψNF hαNF hτφ hτψ _ _ ih₁ ih₂ =>
      intro hmem
      have P₁ := Provable.wk (inv1Push (∀⁰ φ₀) _ χ Γ₀) (ih₁ (Finset.mem_insert_of_mem hmem))
      have P₂ := Provable.wk (inv1Push (∀⁰ φ₀) _ (∼χ) Γ₀) (ih₂ (Finset.mem_insert_of_mem hmem))
...
control-ordinal witness-bounded calculus over the **norm-carrying** `ProvableSlack` wrapper. Cut the
∀-inversion family `fam` (over `φ`, control `e`, index `(k₀,dd₀)`) against an ∃-side derivation
`D : Provable γ e k dd c Δ` containing `∃∼φ`, producing a `Provable`-derivation of `Δ.erase(∃∼φ) ∪ Γ` at
...
theorem cutReduceAllAux {φ : ArithmeticSemiformula ℕ 1} {c k₀ dd₀ : ℕ} {α e : ONote} {Γ : Finset (ArithmeticFormula ℕ)}
    ...
    ∀ {γ : ONote} {k dd : ℕ} {Δ : Finset (ArithmeticFormula ℕ)}, Provable γ e k dd c Δ → γ.NF → norm γ < k + dd →
      k₀ ≤ k → dd₀ ≤ dd → (∃⁰ ∼φ) ∈ Δ →
  ...
  | axL r v hp hn =>
      ...
      exact ⟨0, le_def.mpr (by simp), NF.zero, by simp only [norm_zero]; omega, Provable.axL r v
  ...
  | verumR h =>
      intro hγNF hγb hk hdd hmem
      exact ⟨0, le_def.mpr (by simp), NF.zero, by simp only [norm_zero]; omega, Provable.verumR
        (Finset.mem_union_left _ (Finset.mem_erase.mpr ⟨Semiformula.ne_of_ne_complexity (by simp), h⟩))⟩
  | trueRel r v htrue hτ hαNF' hmemA =>
      intro hγNF hγb hk hdd hmem
      refine ⟨_, le_trans (Provable.le_add_left_NF hαNF hγNF) (le_of_lt (Provable.lt_osucc (ONote.add_nf α _))),
...
theorem embedding_valueCongruentNrelSubstAtom_probe {α e : ONote} {k d c ar n : ℕ} {Γ : Finset (ArithmeticFormula ℕ)}
    (r : (ℒₒᵣ).Rel ar) (w w' : Fin n → ArithmeticTerm ℕ)
    (v : Fin ar → ArithmeticSemiterm ℕ n)
    (hval : ∀ i, stdClosedVal (w i) = stdClosedVal (w' i))
    (hαNF : α.NF) (hτ : norm α < k + d)
