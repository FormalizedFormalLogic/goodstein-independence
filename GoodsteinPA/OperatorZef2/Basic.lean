...
theorem mono_f (dd : Zef2 α e H f c Γ) {f' : ℕ → ℕ} (hff' : ∀ x, f x ≤ f' x) :
    Zef2 α e H f' c Γ := by
...
theorem change_H (dd : Zef2 α e H f c Γ) {H' : ONote → Prop} : Zef2 α e H' f c Γ := by
...
theorem toZef (dd : Zef2 α e H f c Γ) : Zef α e H f c Γ := by
  induction dd with
