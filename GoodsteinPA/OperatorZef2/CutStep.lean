...
private lemma lt_add_of_inner_lt {α β γ : ONote} (hαNF : α.NF) (hγNF : γ.NF) (hβ : β < γ) :
    α < α + γ := by
...
private lemma cutRun_axL {γ : ONote} {Δ : Finset (ArithmeticFormula ℕ)} {ar : ℕ}
...
private lemma cutRun_wk {γ : ONote} {Δsub Δsup : Finset (ArithmeticFormula ℕ)}
...
private lemma cutRun_weak {γ β : ONote} {Δsub Δsup : Finset (ArithmeticFormula ℕ)}
    (hαNF : α.NF) (hg_infl : ∀ x, x ≤ g x)
...
private lemma cutRun_allω {γ : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
    (hαNF : α.NF) (hg0 : Nlog α ≤ g 0)
...
private lemma cutRun_exI {γ β : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
...
private lemma cutRun_cut {γ βφ βψ : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
...
- [EW12, Lemma 25]
-/
lemma cutReduceAllAuxRunning_Zf2 {γ : ONote} {Δ : Finset (ArithmeticFormula ℕ)}
...
private lemma f_le_rel1_2 (hf : Monotone f) (n₀ x : ℕ) : f x ≤ rel1 f n₀ x := hf (le_max_right n₀ x)

/-- Transport a gate `Nlog α ≤ f 0` to the relativized slot `rel1 f n₀`. -/
private lemma gate_rel1 (hmono : Monotone f) (n₀ : ℕ)
...
lemma stepAllω_Zf2 {βφ βψ : ONote}
...
lemma stepAllω_Zf2_bnd {P₁ P₂ : ONote}
    (hP₁ : P₁.NF) (hP₂ : P₂.NF)
    (hENF : E.NF) (hχc : χ.complexity < c)
    (hg_mono : Monotone g) (hg_infl : ∀ x, x ≤ g x)
    (hg_slack : ∀ k, f 0 ≤ k → max (g 0) k + 1 ≤ g k)
...
lemma inertForm_falsum : InertForm ⊥ :=
  ⟨fun _ _ _ => ⟨nofun, nofun⟩, fun _ => ⟨nofun, nofun⟩⟩

lemma inertForm_and (φ₁ φ₂ : ArithmeticFormula ℕ) : InertForm (φ₁ ⋏ φ₂) :=
  ⟨fun _ _ _ => ⟨nofun, nofun⟩, fun _ => ⟨nofun, nofun⟩⟩

lemma inertForm_or (φ₁ φ₂ : ArithmeticFormula ℕ) : InertForm (φ₁ ⋎ φ₂) :=
  ⟨fun _ _ _ => ⟨nofun, nofun⟩, fun _ => ⟨nofun, nofun⟩⟩
...
lemma Zef2.erase_inert {A : ArithmeticFormula ℕ} (hA : InertForm A)
...
private lemma atomRun_axL {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
...
private lemma atomRun_wk {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
...
private lemma atomRun_weak {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
...
private lemma atomRun_allω {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
...
private lemma atomRun_exI {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
...
private lemma atomRun_cut {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
...
lemma atomCutRun_Zf2 {ar : ℕ} {rr : (ℒₒᵣ).Rel ar} {vv : Fin ar → Semiterm ℒₒᵣ ℕ 0}
