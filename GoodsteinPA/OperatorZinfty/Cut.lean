...
variable {φ : ArithmeticSemiformula ℕ 1} {α : ONote} {k₀ dd₀ : ℕ} {Γ : Finset (ArithmeticFormula ℕ)}
  ...
  (hφc : φ.complexity < c') (heNF : e'.NF) (fam : ∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ))
...
private theorem cutReduceAllAux_axL {γ' : ONote} {Δ : Finset (ArithmeticFormula ℕ)} {ar}
    ...
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' (Δ.erase (∃⁰ ∼φ) ∪ Γ) :=
  ⟨0, le_def.mpr (by simp), NF.zero, by simp only [norm_zero]; omega, Zekd.axL r v
...
private theorem cutReduceAllAux_verumR {γ' : ONote} {Δ : Finset (ArithmeticFormula ℕ)}
    ...
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' (Δ.erase (∃⁰ ∼φ) ∪ Γ) :=
  ⟨0, le_def.mpr (by simp), NF.zero, by simp only [norm_zero]; omega, Zekd.verumR
...
private theorem cutReduceAllAux_trueRel {γ' : ONote} {Δ : Finset (ArithmeticFormula ℕ)} {ar}
    ...
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' (Δ.erase (∃⁰ ∼φ) ∪ Γ) :=
  ⟨_, le_trans (Zekd.le_add_left_NF hαNF hγNF) (le_of_lt (Zekd.lt_osucc (ONote.add_nf α _))),
...
private theorem cutReduceAllAux_wk {γ' : ONote} {Δsub Δsup : Finset (ArithmeticFormula ℕ)}
    (hsub : Δsub ⊆ Δsup) (D' : Zekd γ' e' k' dd' c' Δsub)
    (ih : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      γ'.NF → norm γ' < k' + dd' → k₀ ≤ k' → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ Δsub →
      ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' (Δsub.erase (∃⁰ ∼φ) ∪ Γ))
    (hγNF : γ'.NF) (hγb : norm γ' < k' + dd') (hk : k₀ ≤ k') (hdd : dd₀ ≤ dd') (_hmem : (∃⁰ ∼φ) ∈ Δsup) :
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' (Δsup.erase (∃⁰ ∼φ) ∪ Γ) := by
  by_cases hd : (∃⁰ ∼φ) ∈ Δsub
...
private theorem cutReduceAllAux_weak {γ' β : ONote} {Δsub Δsup : Finset (ArithmeticFormula ℕ)}
    ...
    (D' : Zekd β e' k' dd' c' Δsub)
    (ih : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      β.NF → norm β < k' + dd' → k₀ ≤ k' → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ Δsub →
      ZekdProv (osucc (α + β)) e' k' (dd' + norm α + 1) c' (Δsub.erase (∃⁰ ∼φ) ∪ Γ))
...
private theorem cutReduceAllAux_andI {γ' βφ βψ : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
    ...
    (ihφ : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      βφ.NF → norm βφ < k' + dd' → k₀ ≤ k' → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ insert ψ₁ Γ₀ →
      ZekdProv (osucc (α + βφ)) e' k' (dd' + norm α + 1) c' ((insert ψ₁ Γ₀).erase (∃⁰ ∼φ) ∪ Γ))
    (ihψ : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      βψ.NF → norm βψ < k' + dd' → k₀ ≤ k' → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ insert ψ₂ Γ₀ →
      ZekdProv (osucc (α + βψ)) e' k' (dd' + norm α + 1) c' ((insert ψ₂ Γ₀).erase (∃⁰ ∼φ) ∪ Γ))
    (hγNF : γ'.NF) (hγb : norm γ' < k' + dd') (hk : k₀ ≤ k') (hdd : dd₀ ≤ dd') (hmem : (∃⁰ ∼φ) ∈ insert (ψ₁ ⋏ ψ₂) Γ₀) :
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' ((insert (ψ₁ ⋏ ψ₂) Γ₀).erase (∃⁰ ∼φ) ∪ Γ) := by
...
private theorem cutReduceAllAux_orI {γ' β : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
    ...
    (ih : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      ...
      ZekdProv (osucc (α + β)) e' k' (dd' + norm α + 1) c' ((insert ψ₁ (insert ψ₂ Γ₀)).erase (∃⁰ ∼φ) ∪ Γ))
    (hγNF : γ'.NF) (hγb : norm γ' < k' + dd') (hk : k₀ ≤ k') (hdd : dd₀ ≤ dd') (hmem : (∃⁰ ∼φ) ∈ insert (ψ₁ ⋎ ψ₂) Γ₀) :
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' ((insert (ψ₁ ⋎ ψ₂) Γ₀).erase (∃⁰ ∼φ) ∪ Γ) := by
...
private theorem cutReduceAllAux_allω {γ' : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
    ...
    (ih : ∀ n, φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      (β n).NF → norm (β n) < max k' n + dd' → k₀ ≤ max k' n → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ insert (χ/[nm n]) Γ₀ →
      ZekdProv (osucc (α + β n)) e' (max k' n) (dd' + norm α + 1) c'
        ((insert (χ/[nm n]) Γ₀).erase (∃⁰ ∼φ) ∪ Γ))
    (hγNF : γ'.NF) (hγb : norm γ' < k' + dd') (hk : k₀ ≤ k') (hdd : dd₀ ≤ dd') (hmem : (∃⁰ ∼φ) ∈ insert (∀⁰ χ) Γ₀) :
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' ((insert (∀⁰ χ) Γ₀).erase (∃⁰ ∼φ) ∪ Γ) := by
...
private theorem cutReduceAllAux_exI {γ' β : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
    ...
    (dχ : Zekd β e' k' dd' c' (insert (χ/[nm n]) Γ₀))
    (ih : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      β.NF → norm β < k' + dd' → k₀ ≤ k' → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ insert (χ/[nm n]) Γ₀ →
      ZekdProv (osucc (α + β)) e' k' (dd' + norm α + 1) c' ((insert (χ/[nm n]) Γ₀).erase (∃⁰ ∼φ) ∪ Γ))
    (hγNF : γ'.NF) (hγb : norm γ' < k' + dd') (hk : k₀ ≤ k') (hdd : dd₀ ≤ dd') (hmem : (∃⁰ ∼φ) ∈ insert (∃⁰ χ) Γ₀) :
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' ((insert (∃⁰ χ) Γ₀).erase (∃⁰ ∼φ) ∪ Γ) := by
  have hsuccNF : (osucc (α + γ')).NF := osucc_NF (ONote.add_nf α γ')
  by_cases hhd : (∃⁰ χ) = (∃⁰ ∼φ)
...
private theorem cutReduceAllAux_cut {γ' βφ βψ : ONote} {Γ₀ : Finset (ArithmeticFormula ℕ)}
    ...
    (ih₁ : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      ...
      ZekdProv (osucc (α + βφ)) e' k' (dd' + norm α + 1) c' ((insert χ Γ₀).erase (∃⁰ ∼φ) ∪ Γ))
    (ih₂ : φ.complexity < c' → e'.NF → (∀ n, Zekd α e' k₀ dd₀ c' (insert (φ/[nm n]) Γ)) →
      βψ.NF → norm βψ < k' + dd' → k₀ ≤ k' → dd₀ ≤ dd' → (∃⁰ ∼φ) ∈ insert (∼χ) Γ₀ →
      ZekdProv (osucc (α + βψ)) e' k' (dd' + norm α + 1) c' ((insert (∼χ) Γ₀).erase (∃⁰ ∼φ) ∪ Γ))
    (hγNF : γ'.NF) (hγb : norm γ' < k' + dd') (hk : k₀ ≤ k') (hdd : dd₀ ≤ dd') (hmem : (∃⁰ ∼φ) ∈ Γ₀) :
    ZekdProv (osucc (α + γ')) e' k' (dd' + norm α + 1) c' (Γ₀.erase (∃⁰ ∼φ) ∪ Γ) := by
