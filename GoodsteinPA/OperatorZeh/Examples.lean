...
lemma finite_part_unbounded (S : ONote → Prop) (m : ℕ) : Cl S (ONote.ofNat m) :=
  Cl.ofNat m

/-- The additive raise genuinely absorbs a numeral base (kernel-computed):
`raise (ofNat 5) 1 = ofNat 5 + ω = ω`. -/
lemma raise_absorbs_base : raise (ONote.ofNat 5) 1 = ONote.omega := rfl
...
lemma mono_e_membership_gate_refuted :
...
lemma norm_ball_not_add_closed (R : ℕ) (hR : 1 ≤ R) :
...
lemma concrete_bound_computes : hardy ONote.omega 1 = 3 := by
...
lemma normControlled_exists_trivial (e : ONote) (m : ℕ) : ∃ f : ℕ → ℕ, NormControlled f e m :=
...
lemma seam1_bump_absorbed_by_composition (x : ℕ) : ∃ g : ℕ → ℕ, ∀ dd : ℕ, dd + x + 1 ≤ g dd :=
...
lemma seam2_function_slot_payable (dBase eNorm : ℕ) :
