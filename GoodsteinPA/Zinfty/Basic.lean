...
Definitions of the infinitary ω-rule calculus `Z_∞` over Foundation's real PA syntax
(`SyntacticFormula ℒₒᵣ`): the closed formulas `Form`, numerals `nm`, set sequents `Seq`,
signed literals `signedLit`/`LitTrue`, the calculus `Deriv` (Towsner [Tow20] §13), and the
derivation measures `ordinalBound` (§15) / `cutRank` (§18) with the bounded-provability
predicate `Provable` (§18). The cut-elimination lemmas built on top of these live in
`GoodsteinPA.Zinfty`.
-/
module
