...
- [Tow20, §19.4] -/
lemma allInvAux (n : ℕ) (D : Derivation Γ) (hcr : cutRank D ≤ (c : ℕ∞)) (hmem : (∀⁰ φₓ) ∈ Γ) :
  Provable (ordinalBound D) c (insert (φₓ/[nm n]) (Γ.erase (∀⁰ φₓ))) := by
  induction D with
  | @axL Γ k r v hp hn =>
    exact (Provable.axL r v (by grind) (by grind)).mono le_rfl (by omega)
  | @axTrue Γ k b r v htrue hmem =>
  ...
  | @weak Δ Γ D' hsub ih =>
    ...
    · have : Provable (ordinalBound D') c Δ := ⟨D', le_rfl, hcr⟩
      ...
      grind;
  | @andI Γ₀ φ' ψ' Dφ Dψ ihφ ihψ =>
    apply (Provable.andI ?_ ?_).weakening $ invPull1 _ (by grind) Γ₀;
    . exact ihφ (le_trans (le_max_left _ _) hcr) (by grind) |>.weakening $ invPush1 _ φ' _ Γ₀;
    . exact ihψ (le_trans (le_max_right _ _) hcr) (by grind) |>.weakening $ invPush1 _ ψ' _ Γ₀;
  | @orI Γ₀ φ' ψ' D' ih =>
    apply (Provable.orI ?_).weakening $ invPull1 _ (by grind) Γ₀;
    apply (ih hcr (by grind)).weakening;
    intro χ;
    simp only [Finset.mem_insert, Finset.mem_erase];
    grind;
  | @allω Γ₀ χ' Dₓ ih =>
    ...
    · obtain rfl := (Semiformula.all_inj _ _).mp hhd;
      have hcrn : cutRank (Dₓ n) ≤ (c : ℕ∞) := le_trans (le_iSup (fun m => cutRank (Dₓ m)) n) hcr;
      have hbound : ordinalBound (Dₓ n) ≤ (⨆ m, ordinalBound (Dₓ m)) + 1 :=
        le_trans (Ordinal.le_iSup (fun m => ordinalBound (Dₓ m)) n) (le_of_lt (lt_add_of_pos_right _ one_pos));
      by_cases hd : (∀⁰ χ') ∈ Γ₀;
      · apply ((ih n hcrn (Finset.mem_insert_of_mem hd)).weakening ?_).mono hbound le_rfl;
        intro χ;
        simp only [Finset.mem_insert, Finset.mem_erase];
        grind;
      · have base : Provable (ordinalBound (Dₓ n)) c (insert (χ'/[nm n]) Γ₀) := ⟨Dₓ n, le_rfl, hcrn⟩;
        apply (base.weakening ?_).mono hbound le_rfl;
        intro χ;
        simp only [Finset.mem_insert, Finset.mem_erase];
        grind;
    · apply (Provable.allω ?_).weakening $ invPull1 _ hhd Γ₀;
      intro m;
      exact (ih m (le_trans (le_iSup (fun j => cutRank (Dₓ j)) m) hcr) (by grind)).weakening
        (invPush1 _ (χ'/[nm m]) _ Γ₀);
  | @exI Γ₀ χ' m Dₓ ih =>
    apply (Provable.exI m ?_).weakening (invPull1 _ (by grind) Γ₀);
    exact (ih hcr (by grind)).weakening (invPush1 _ (χ'/[nm m]) _ Γ₀);
  | @cut Γ₀ ξ D₁ D₂ ih₁ ih₂ =>
    apply Provable.cut ξ;
    . exact (le_max_left _ _).trans hcr
    . apply ih₁ ?_ (by grind) |>.weakening (invPush1 _ ξ _ Γ₀);
      exact (le_max_left (cutRank D₁) (cutRank D₂)).trans ((le_max_right _ _).trans hcr);
    . apply ih₂ ?_ (by grind) |>.weakening (invPush1 _ (∼ξ) _ Γ₀);
      exact (le_max_right (cutRank D₁) (cutRank D₂)).trans ((le_max_right _ _).trans hcr);
...
- [Tow20, §19.3] -/
lemma andInvAux (D : Derivation Γ) (hcr : cutRank D ≤ (c : ℕ∞)) (hmem : (φ ⋏ ψ) ∈ Γ) :
    Provable (ordinalBound D) c (insert φ (Γ.erase (φ ⋏ ψ))) ∧
    Provable (ordinalBound D) c (insert ψ (Γ.erase (φ ⋏ ψ))) := by
  induction D with
  | @axL Γ k r v hp hn =>
    exact ⟨(Provable.axL r v (by grind) (by grind)).mono le_rfl (by omega),
           (Provable.axL r v (by grind) (by grind)).mono le_rfl (by omega)⟩
  | @axTrue Γ k b r v htrue hmem =>
    refine ⟨(Provable.axTrue b r v htrue ?_).mono le_rfl (by omega),
            (Provable.axTrue b r v htrue ?_).mono le_rfl (by omega)⟩;
    . cases b <;> . simp [signedLit]; grind;
    . cases b <;> . simp [signedLit]; grind;
  | @verumR Γ h =>
    exact ⟨(Provable.verumR (by grind)).mono le_rfl (by omega),
           (Provable.verumR (by grind)).mono le_rfl (by omega)⟩
  | @weak Δ Γ D' hsub ih =>
    by_cases hd : (φ ⋏ ψ) ∈ Δ
  ...
  | @andI Γ₀ φ' ψ' Dφ Dψ ihφ ihψ =>
    ...
    · obtain ⟨rfl, rfl⟩ := (Semiformula.and_inj _ _ _ _).mp hhd.symm
      ...
      refine ⟨?_, ?_⟩
      · by_cases hd : (φ ⋏ ψ) ∈ Γ₀
        · apply ((ihφ hcrφ (Finset.mem_insert_of_mem hd)).1.weakening ?_).mono hbφ le_rfl;
          intro χ; simp only [Finset.mem_insert, Finset.mem_erase]; grind
        · have base : Provable (ordinalBound Dφ) c (insert φ Γ₀) := ⟨Dφ, le_rfl, hcrφ⟩
          apply (base.weakening ?_).mono hbφ le_rfl;
          intro χ; simp only [Finset.mem_insert, Finset.mem_erase]; grind
      · by_cases hd : (φ ⋏ ψ) ∈ Γ₀
        · apply ((ihψ hcrψ (Finset.mem_insert_of_mem hd)).2.weakening ?_).mono hbψ le_rfl;
          intro χ; simp only [Finset.mem_insert, Finset.mem_erase]; grind
        · have base : Provable (ordinalBound Dψ) c (insert ψ Γ₀) := ⟨Dψ, le_rfl, hcrψ⟩
          apply (base.weakening ?_).mono hbψ le_rfl;
          intro χ; simp only [Finset.mem_insert, Finset.mem_erase]; grind
    · refine ⟨?_, ?_⟩
      · exact (Provable.andI ((ihφ hcrφ (by grind)).1.weakening (invPush1 _ φ' _ Γ₀))
                             ((ihψ hcrψ (by grind)).1.weakening (invPush1 _ ψ' _ Γ₀))).weakening (invPull1 _ hhd Γ₀)
      · exact (Provable.andI ((ihφ hcrφ (by grind)).2.weakening (invPush1 _ φ' _ Γ₀))
                             ((ihψ hcrψ (by grind)).2.weakening (invPush1 _ ψ' _ Γ₀))).weakening (invPull1 _ hhd Γ₀)
  | @orI Γ₀ φ' ψ' D' ih =>
    refine ⟨?_, ?_⟩
    · apply (Provable.orI ?_).weakening $ invPull1 _ (by grind) Γ₀;
  ...
  | @allω Γ₀ χ' Dₓ ih =>
    refine ⟨?_, ?_⟩
    · apply (Provable.allω ?_).weakening $ invPull1 _ (by grind) Γ₀;
      intro m;
      exact (ih m (le_trans (le_iSup (fun j => cutRank (Dₓ j)) m) hcr) (by grind)).1.weakening
        (invPush1 _ (χ'/[nm m]) _ Γ₀)
    · apply (Provable.allω ?_).weakening $ invPull1 _ (by grind) Γ₀;
      intro m;
      exact (ih m (le_trans (le_iSup (fun j => cutRank (Dₓ j)) m) hcr) (by grind)).2.weakening
        (invPush1 _ (χ'/[nm m]) _ Γ₀)
  | @exI Γ₀ χ' m Dₓ ih =>
    refine ⟨?_, ?_⟩
    · apply (Provable.exI m ?_).weakening (invPull1 _ (by grind) Γ₀);
      exact (ih hcr (by grind)).1.weakening (invPush1 _ (χ'/[nm m]) _ Γ₀)
    · apply (Provable.exI m ?_).weakening (invPull1 _ (by grind) Γ₀);
      exact (ih hcr (by grind)).2.weakening (invPush1 _ (χ'/[nm m]) _ Γ₀)
  | @cut Γ₀ ξ D₁ D₂ ih₁ ih₂ =>
    refine ⟨?_, ?_⟩
    · apply Provable.cut ξ;
      . exact (le_max_left _ _).trans hcr
      . apply (ih₁ ?_ (by grind)).1.weakening (invPush1 _ ξ _ Γ₀);
        exact (le_max_left (cutRank D₁) (cutRank D₂)).trans ((le_max_right _ _).trans hcr)
      . apply (ih₂ ?_ (by grind)).1.weakening (invPush1 _ (∼ξ) _ Γ₀);
        exact (le_max_right (cutRank D₁) (cutRank D₂)).trans ((le_max_right _ _).trans hcr)
    · apply Provable.cut ξ;
      . exact (le_max_left _ _).trans hcr
      . apply (ih₁ ?_ (by grind)).2.weakening (invPush1 _ ξ _ Γ₀);
        exact (le_max_left (cutRank D₁) (cutRank D₂)).trans ((le_max_right _ _).trans hcr)
      . apply (ih₂ ?_ (by grind)).2.weakening (invPush1 _ (∼ξ) _ Γ₀);
        exact (le_max_right (cutRank D₁) (cutRank D₂)).trans ((le_max_right _ _).trans hcr)
