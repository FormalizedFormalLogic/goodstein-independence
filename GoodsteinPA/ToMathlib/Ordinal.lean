...
theorem max_add_add_one_add_one_le (α a b : Ordinal) :
...
theorem add_add_one_add_one_le (α a : Ordinal) : α + a + 1 + 1 ≤ α + (a + 1) + 1 :=
  le_of_eq (by rw [add_assoc α a 1])

/-- Bound bookkeeping for the ω-rule commuting case. -/
theorem iSup_add_add_one_add_one_le (α : Ordinal) (f : ℕ → Ordinal) :
...
theorem one_lt_opow_succ (c : Ordinal) : (1 : Ordinal) < Ordinal.omega0 ^ (c + 1) := by
...
theorem opow_lt_opow_succ_of_le_max {a b x : Ordinal}
...
theorem max_opow_add_one_le (a b : Ordinal) :
...
theorem max_opow_add_two_le (a b : Ordinal) :
...
theorem opow_add_opow_add_one_le (a b : Ordinal) :
...
theorem opow_add_one_le' (a : Ordinal) :
...
theorem sup_opow_add_one_le (f : ℕ → Ordinal) :
