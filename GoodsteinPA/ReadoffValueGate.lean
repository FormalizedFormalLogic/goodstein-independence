...
variable {P : ℕ → ℕ} {V : ℕ} {χ₁ χ₂ : ArithmeticFormula ℕ} {φ₀ : ArithmeticSemiformula ℕ 1} {k : ℕ}

theorem Gated_and_iff : Gated P V (χ₁ ⋏ χ₂) ↔ Gated P V χ₁ ∧ Gated P V χ₂ := by
...
theorem Gated_mono (hP : Monotone P) :
...
theorem tvB_nm (k B : ℕ) : tvB (nm k) B = k := by
  ...
  simp only [tvB, he]
  exact ArithmeticTerm.valm_nm k (fun _ => 0)
...
theorem gate_extract {t : Semiterm ℒₒᵣ ℕ 1} (hpos : t.Positive)
  ...
  refine ⟨?_, by
    simpa [atomTrue, Semiformula.eval_substs, ArithmeticTerm.valm_nm, Matrix.constant_eq_singleton]
      using h.2⟩
  ...
  have h1 : Semiterm.val (fun _ => 0) (fun _ => 0)
    ...
    exact ArithmeticTerm.valm_nm k _
