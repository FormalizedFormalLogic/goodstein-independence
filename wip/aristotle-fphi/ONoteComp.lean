...
/-
Goal: prove that comparison of Cantor-normal-form ordinal notations (mathlib's `ONote`/`NONote`)
is COMPUTABLE, packaged as a single `REPred` (recursively-enumerable predicate) lemma.

mathlib has `ONote.cmp : ONote → ONote → Ordering` (a structural recursion) and the `LinearOrder NONote`
it induces, but provides NO `Computable`/`Primrec` proof for it. This file supplies the structural
`Encodable ONote` (a pairing) and the `Denumerable NONote` it yields, then asks for the final fact.

The intended crux: build `Primcodable ONote` from the STRUCTURAL encoding below (NOT from
`Primcodable.ofDenumerable`, whose enumeration order is structurally opaque), prove `ONote.cmp`
is `Primrec₂` by the structural recursion, transfer to `NONote`, and conclude.

The final theorem `rePred_ltPull_natCode` is the ONLY thing that must be proved (no `sorry`).
Everything above it is fixed scaffolding (do not change the statements of the `def`s/instances).
-/
...
/-- The concrete computable coding `ℕ ≃ NONote`. -/
...
theorem encode_decode_eq (n : ℕ) :
  ...
  rw [ ONoteComp.decode_eq ];
  simp +decide [ Encodable.encode, ONoteComp.encode_eq, encodeONote_decodeONote ]
...
theorem ordCode_cmp (a b : ℕ) : ordCode (_root_.cmp a b) = cmpNat a b := by
  by_cases ha : a < b <;> by_cases hb : a = b <;> simp +decide [ ha, hb, _root_.cmp ];
  · linarith;
  · simp +decide [ cmpUsing, ha, cmpNat ];
  · unfold cmpNat; simp +decide [ cmpUsing ] ;
  · simp +decide [ ha, hb, cmpUsing, cmpNat ];
    rw [ if_pos ( lt_of_le_of_ne ( le_of_not_gt ha ) ( Ne.symm hb ) ) ] ; rfl
...
theorem primrec_cmpIdxA : Primrec cmpIdxA := by
  ...
  · convert Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.pred.comp ( Primrec.fst.comp ( Primrec.unpair ) ) ) ) ) ) using 1;
  · exact Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.nat_sub.comp ( Primrec.snd.comp ( Primrec.unpair ) ) ( Primrec.const 1 ) ) ) ) )

theorem primrec_cmpNV : Primrec cmpNV := by
  have h_primrec_cmpNV : Primrec₂ (fun x y : ℕ => cmpNat x y) := by
    convert Primrec.ite _ _ _ using 1;
    · exact Primrec.nat_lt.comp ( Primrec.fst ) ( Primrec.snd );
    · exact Primrec.const 0;
    · convert Primrec.ite _ _ _ using 1;
      · exact Primrec.eq;
      · exact Primrec.const 1;
      · exact Primrec.const 2;
  convert h_primrec_cmpNV.comp _ _ using 1;
  · exact Primrec.nat_add.comp ( Primrec.fst.comp ( Primrec.unpair.comp ( Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.nat_sub.comp ( Primrec.fst.comp ( Primrec.unpair ) ) ( Primrec.const 1 ) ) ) ) ) ) ( Primrec.const 1 );
  · exact Primrec.nat_add.comp ( Primrec.fst.comp ( Primrec.unpair.comp ( Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.nat_sub.comp ( Primrec.snd.comp ( Primrec.unpair ) ) ( Primrec.const 1 ) ) ) ) ) ) ( Primrec.const 1 )
...
theorem primrec_thenNat : Primrec₂ thenNat := by
  ...
  exact fun p => if p.1 = 1 then p.2 else p.1;
  · convert Primrec.ite _ _ _ using 1;
    · exact Primrec.eq.comp ( Primrec.fst ) ( Primrec.const 1 );
    · exact Primrec.snd;
    · exact Primrec.fst;
  · aesop
...
/-
**The target.** The order `natCode a < natCode b` on ℕ-codes is recursively enumerable
(in fact computable). PROVE THIS (replace the `sorry`).
-/
theorem rePred_ltPull_natCode :
  ...
  · exact fun v => by rw [ decide_eq_decide.mpr ( ONoteComp.lt_iff_Cnat _ _ |> Iff.symm ) ] ;)
