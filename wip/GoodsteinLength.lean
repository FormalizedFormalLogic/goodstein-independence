/-
# Goodstein termination + length — PORTED from Track-1 (toward `Hdom`)
Verbatim merge of Track-1 Goodstein/{Engine,Statement,Length}.lean (Goodstein defs are
BYTE-IDENTICAL to this repo's `src/GoodsteinPA/Defs.lean`; mathlib rev fabf563a7c95).
Provides `goodstein_terminates` (unconditional, ordinal descent) + `goodsteinLength`, and
the bridge `sInf{m|goodsteinSeq n m=0} = goodsteinLength n` (= `wip/LowerBoundHardy.lean`'s
`G`). Namespace localized to `GoodsteinPA`. WIP — not in build target.
-/
...
-- ════════════════ bridge: `G` (sInf) = `goodsteinLength` (Nat.find) ════════════════

/-- `wip/LowerBoundHardy.lean`'s `G n = sInf {m | goodsteinSeq n m = 0}` is exactly the Goodstein
length: both pick the least zero step (the set is nonempty by `goodstein_terminates`). -/
theorem sInf_zeroSet_eq_goodsteinLength (n : ℕ) :
    sInf {m | goodsteinSeq n m = 0} = goodsteinLength n := by
  apply le_antisymm
  · exact Nat.sInf_le (goodsteinSeq_goodsteinLength n)
  · exact Nat.find_le (Nat.sInf_mem (goodstein_terminates n))
